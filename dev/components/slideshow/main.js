        ...
        if(projects){                                         // Check for projects val existance
          for (var i = 0; i < projects.length; i++) {         // Loop through array of projects
            if(typeof projects[i] == 'object'){               // Check that item is a project object
              ...
              var projectPages = projects[i].pages;                  // Pages param as projectPages var
              if(!projects[i].hasOwnProperty('pages')){       // Project doesn't contain pages
              ...
              } else if (projectPages.length == 1) {                   // Check if there is only one step
                ...
                for (var j = 0; j < projectPages.length; j++) {      // Loop through pages
                  var elementHtmlString = '<section class="reveal_section">';
                  var subSection = null;
                  if(projectPages[j].hasOwnProperty('image')){       //Check for image in page
                    if(!projectPages[j].image.hasOwnProperty('style')){  //No Background styling
                      elementHtmlString = '<section class="reveal_section" data-background="'+ projectPages[j].image.url+'">'//Background url from page
                      subSection = angular.element(elementHtmlString);
                    } else {                                  //Background styling exists
                      elementHtmlString = '<section class="reveal_section">';
                      subSection = angular.element(elementHtmlString);
                      subSection.append('<img style="'+projectPages[j].image.style+'" src="'+ projectPages[j].image.url +'">'); //Append image to subsection
                    }
                  }
